...
console.log("hello world");
		if ($.browser.msie && $.browser.version < 9) {
			...
			this._vml.style.width = parseInt(this._wrapper[0].currentStyle.width) + 1 + "px";
			this._vml.style.height = parseInt(this._wrapper[0].currentStyle.height) + 1 + "px";
			this._doVML();
		...
		$("label[for=\"" + this.element.attr('id') + "\"]")
			...
			.hover(function(event) {
				self._wrapper.addClass("ui-state-hover");
			}, function(event) {
				self._wrapper.removeClass("ui-state-hover");
			});
	},
	...
	// deprecated: use option("checked", true)
	check: function() {
		this._setOption("checked", true);
	},

	// deprecated: use option("checked", false)
	uncheck: function() {
		this._setOption("checked", false);
	},

	// Toggles the state of the checkbox, whichever it is
	toggle: function() {
		this._setOption("checked", !this.options["checked"]);
	},

	// Retrieves the icon class appropriated for this widget (radio or checkbox)
	...
	// Only called for IE 7 and 8
	_doVML: function() {
		if (!this._vml || this._inFixup) {
			return;
		}

		// Use a lock flag to prevent dispatching of events while doing this
		this._inFixup = true;
